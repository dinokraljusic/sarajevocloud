	...
	private void loadModelFromFile() {
		...
		try {
			if (textureFileName != null)
				texture = new Texture(Gdx.files.absolute(textureFileName), true);
		} catch (Exception e) {
		...
		/*else*/ if (fileName.endsWith(".obj"))
			model = new ObjLoader().loadObj(Gdx.files.absolute(fileName), true);
		else if (fileName.endsWith(".g3d"))
			model = G3dLoader.loadStillModel(Gdx.files.absolute(fileName));

		else if (fileName.endsWith(".md2")) {
			keyFramedModel = new MD2Loader().load(Gdx.files.absolute(fileName),
					1 / 7f);
			if (texture != null) {
...
//		}
			...
			keyFramedModel = G3dtLoader.loadKeyframedModel(
					Gdx.files.absolute(fileName), false);
